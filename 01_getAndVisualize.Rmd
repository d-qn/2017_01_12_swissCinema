...
groups <- data.frame(
  ...
  color = c("#ac673e", "#333366", "#666633", "#aa8959", "#336666", "#448888", "#ab3d3f", "#55aaaa", "#3a9736")
)
...
dd$share_3d     <- dd$d3 / dd$nAdmissions 
dd$share_dubbed <- dd$dubbed / dd$nAdmissions

dd$share_ani    <- dd$ani / dd$nFilms
dd$share_doc    <- dd$doc / dd$nFilms
dd$share_fic    <- dd$fic / dd$nFilms
...
myTheme <- function(base_family, title_family) {
  swi_theme(base_family = base_family, title_family = title_family) + 
    theme(
      #panel.ontop = TRUE,
      panel.grid.major.y=element_line(color="black", linetype="dotted", size=0.2),
...
gp <- ggplot(data = ee %>% filter(gr == 1), aes(name, value, fill = color)) + 
  geom_bar_interactive(stat = "identity") + 
  facet_wrap(~ varName, scales = "free_y", ncol = 1) +  
  geom_text(data= filter(ee, var %in% c("nFilms", "admissionByScreening")), aes(label=name), position=position_fill(), vjust=-0.25, colour = "white", size = 2.5) + 
  scale_fill_identity() + 
