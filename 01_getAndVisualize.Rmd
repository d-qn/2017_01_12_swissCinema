  ...
  swi_theme(base_family = base_family) + 
    theme(
     ...
     # plot.background = element_rect(fill = "#efe9e0", color = NA),
      ...
      plot.margin=unit(c(0.3, -0.1, 0, -0.55), "cm")
    ) 
}
...
myBarPlot <- function (eee, base_font, fontFactor = 1, ndigits = 0, annotations = NULL, ...) {
  gp <- ggplot(data = eee, aes(name, value, fill = color)) + 
    ...
    geom_text(aes(label=name), position=position_fill(), vjust=-0.35, hjust = 0.37, 
              colour = "white", size = 2.7, family = base_font, check_overlap = T) + 
    scale_fill_identity() + 
  ...
  save_html(
    tags$html(
      ...
      tags$body(    
        ...
        div(class="container", myIplot(gp, height_svg = 9)),
        h2(HTML(txt["title.b", lang])),
        div(class = "descr", HTML(paste0(txt["descr2", lang]))),
        div(class="container", myIplot(gp2, height_svg = 12)),
