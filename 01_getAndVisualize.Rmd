...
myTheme <- function(base_family) {
  swi_theme(base_family = base_family) + 
    theme(
      #panel.ontop = TRUE,
      panel.grid.major.y=element_line(color="#2b2b2b", linetype="dotted", size=0.03),
      ...
      plot.margin=unit(c(0.3, 0.05, 0, -0.6), "cm")
    ) 
}

myBarPlot <- function (eee, base_font, fontFactor = 1, ndigits = 0, ...) {
  ggplot(data = eee, aes(name, value, fill = color)) + 
    geom_bar_interactive(aes(tooltip = name, data_id = name), stat = "identity") + 
